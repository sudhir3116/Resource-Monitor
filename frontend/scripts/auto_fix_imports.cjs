...
const IMPORT_RULES = [
    ...
    { match: /\buseAuth\b/g, check: 'useAuth } from', add: "import { useAuth } from '{prefix}context/AuthContext';" },
    { match: /\bAuthContext\b/g, check: 'AuthContext } from', add: "import { AuthContext } from '{prefix}context/AuthContext';" },
...
allFiles.forEach(file => {
    ...
    // if relatively same folder (depth 1), prefix is '../'
    const prefix = '../'.repeat(depth);

    let importsToAdd = [];

    IMPORT_RULES.forEach(rule => {
        if (content.match(rule.match) && !content.includes(rule.check) && !content.includes(`import { ${rule.check.trim()}`)) {
    ...
    ['useNavigate', 'NavLink', 'Link'].forEach(comp => {
        ...
        if (content.match(rx)) {
            if (!content.includes(`import { ${comp}`) && !content.includes(comp + ' } from') && !content.includes(comp + ',') && !content.includes(' ' + comp + ' }') && !content.includes(`import {${comp}`)) {
    ...
    // Lucide React
    if (content.match(/<([A-Z][A-Za-z0-9]*)/g)) {
        // It's too complex to statically analyze, but let's check for `lucide-react` import
    }

    if (importsToAdd.length > 0) {
        const newImports = importsToAdd.join('\n') + '\n';
        // find first import to insert after, or if no import, insert at the beginning
